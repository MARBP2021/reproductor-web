...
const Login = ({ goToRegister, goToPlay, capturarId }) => {
  ...
  const [login, setIngreso] = useState({ email: "", password: "" });
  ...
  const traerDatos = async (email, password) => {
    ...
    await fetch(url, options)
      ...
      .then(function (data) {
        ...
        // Enviando el id y usuario hacia APP, asi el usuario podra ingresar
        if (user && id) {
        ...
        <form className="form" onSubmit={verificarIngreso}>
          <div>
            <div className="form__grid">
              <span className={datosIncorrectos}>
                Los datos ingresados son incorrectos
              </span>
              <span className={camposVacios}>
